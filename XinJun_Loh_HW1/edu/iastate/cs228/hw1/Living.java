...
public abstract class Living 
{
	...
	{		
		...
		for(int i=Math.max(this.row-1, 0); i<=Math.min(plain.getWidth()-1, this.row+1); i++) {
			for(int j=Math.max(this.column-1, 0); j<=Math.min(plain.getWidth()-1, this.column+1); j++) {
				int count=-1;
